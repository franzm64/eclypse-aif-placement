"""
UC2-Capable comparison plot: BestFit vs AIF under calendar load with
redesigned infrastructure (capable nodes, realistic failure rate).

Infrastructure: 187-node hierarchical, kill=0.5%, revive~N(10%,2%) → ~5% dead.
Application: SockShop with 2× FrontendService replicas, cpu/ram/availability
requirements — dead nodes automatically excluded from placement.
...
BESTFIT_CSV = Path("results/uc2_capable_bestfit-20260828_003335/csv")
AIF_CSV     = Path("results/uc2_capable_aif-20260828_003335/csv")

FIG_DIR = Path("results/uc2_capable/figures")
...
fig.suptitle(
    "UC2-Capable: BestFit vs Active Inference — capable nodes, realistic failure rate\n"
    f"(187-node hierarchical, cpu 2–8 cores, ram 3–8 GB, {N_MONTHS} monthly cycles, "
...
# Unreachability is shown raw (no smoothing) so that brief 100% spikes in
# BestFit are visible.  AIF's bounded behaviour is the contrasting story.
...
ax_mid.plot(ticks, if_bf.values * 100, color=C_BLUE,  lw=1.2, zorder=3,
            label="BestFit unreachable")
ax_mid.plot(ticks, if_ai.values * 100, color=C_GREEN, lw=1.2, zorder=3,
            label="AIF unreachable")

ax_mid.set_ylabel("Nodes with\napp unreachable (%)")
ax_mid.set_ylim(0, min(105, max(if_bf.max(), if_ai.max()) * 100 * 1.15))
...
plt.tight_layout()
out = FIG_DIR / "fig_uc2_capable_bestfit_vs_aif.png"
...
# Auto-scale the unreachable axis to the actual data
max_inf = max(if_bf.max(), if_ai.max()) * 100
print(f"\nPeak unreachable: BestFit {if_bf.max()*100:.1f}%  AIF {if_ai.max()*100:.1f}%")
