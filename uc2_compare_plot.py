"""
UC2-Calendar comparison plot: BestFit baseline vs AIF placement.
...
# ── paths ─────────────────────────────────────────────────────────────────────
BESTFIT_CSV = Path("results/uc2_calendar-20260823_174257/csv")
AIF_CSV     = Path("results/uc2_aif-20260823_225435/csv")
...
def _load(csv_dir: Path):
    """Return (phase_map, by_tick, inf_frac) from a results CSV directory."""
    # Find the timestamped subdirectory if csv_dir is a base path
    if not (csv_dir / "simulation.csv").exists():
        # Look for latest timestamped subdir
        candidates = sorted(csv_dir.parent.glob(f"{csv_dir.name}*/csv"))
        if not candidates:
            raise FileNotFoundError(f"No CSV data found under {csv_dir}")
        csv_dir = candidates[-1]
...
fig.suptitle(
    "UC2-Calendar: BestFit vs Active Inference — user delay & reachability\n"
    f"(187-node hierarchical, SockShop ×2 replicas, {N_MONTHS} monthly cycles, "
    "kill prob 2 %)",
    fontsize=12, fontweight="bold",
)

# ── holiday shading and deadline vlines ───────────────────────────────────────
...
ax_top.plot(ticks, mean_bf.values,       color=C_BLUE,   lw=2.0, zorder=3,
            label="BestFit (rolling avg)")
ax_top.plot(ticks, mean_ai.values,       color=C_GREEN,  lw=2.0, zorder=3,
            label="AIF (rolling avg)")
...
# ── middle panel ──────────────────────────────────────────────────────────────
ax_mid.fill_between(ticks, if_bf.values * 100,
                    color=C_BLUE,   alpha=0.18, zorder=2)
ax_mid.fill_between(ticks, inf_ai.values * 100,
                    color=C_GREEN,  alpha=0.18, zorder=2)
...
plt.tight_layout()
out = FIG_DIR / "fig_uc2_compare_bestfit_aif.png"
fig.savefig(out, dpi=150, bbox_inches="tight")
plt.close(fig)
print(f"Saved {out}")

# ── summary table ─────────────────────────────────────────────────────────────
print("\nMean user_delay by strategy (finite observations, all phases):")
print(f"  BestFit : {bt_bf['mean'].mean():.1f} ms")
print(f"  AIF     : {bt_ai['mean'].mean():.1f} ms")

print("\nMean unreachable fraction by strategy:")
