"""
UC2-Recovery comparison plot: BestFit vs AIF under oscillating kill/recovery.

Three-panel layout (shared x-axis):
  Top    : mean user_delay — BestFit (blue) vs AIF (green), rolling averages
  Middle : unreachable fraction — BestFit (blue) vs AIF (green)
...
# ── paths — update after both experiments complete ─────────────────────────────
BESTFIT_CSV = Path("results/uc2_recovery_bestfit-20260824_000605/csv")
AIF_CSV     = Path("results/uc2_recovery_aif-20260824_000610/csv")

FIG_DIR = Path("results/uc2_recovery/figures")
...
def _resolve_csv(csv_dir: Path) -> Path:
    if (csv_dir / "simulation.csv").exists():
        return csv_dir
...
fig.suptitle(
    "UC2-Recovery: BestFit vs Active Inference — oscillating kill/recovery\n"
...
plt.tight_layout()
out = FIG_DIR / "fig_uc2_recovery_bestfit_vs_aif.png"
