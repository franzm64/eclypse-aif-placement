"""
UC2-Capable 12-month comparison plot: BestFit vs AIF.

Three-panel layout (shared x-axis):
  Top    : mean user_delay — BestFit (blue) vs AIF (green), 30-tick rolling avg
...
BESTFIT_CSV = Path("results/uc2_capable_12m_bestfit-20260831_202926/csv")
AIF_CSV     = Path("results/uc2_capable_12m_aif-20260831_202926/csv")

FIG_DIR = Path("results/uc2_capable_12m/figures")
...
fig.suptitle(
    "UC2-Capable 12-month: BestFit vs Active Inference — capable nodes, realistic failure rate\n"
    "(187-node hierarchical, cpu 2–8 cores, ram 3–8 GB, "
    "kill=0.5%, revive~N(10%,2%) → ~5% dead equilibrium)",
    ...
    for d in range(1, DAYS_PER_MONTH + 1):
        if d in HOLIDAY_DAYS:
            ...
            for ax in (ax_top, ax_mid):
                ax.axvspan(s, e, color=C_GRAY, alpha=0.12, zorder=0)
    deadline_t = mo + 14 * TICKS_PER_DAY
    for ax in (ax_top, ax_mid):
        ax.axvline(deadline_t, color=C_RED, linestyle="--",
                   linewidth=0.8, alpha=0.60, zorder=1,
                   label="Deadline (day 15)" if m == 0 else "_")

# Top: smoothed latency
ax_top.plot(ticks, bt_bf["mean"].values, color=C_BLUE,  lw=0.4, alpha=0.25, zorder=2)
ax_top.plot(ticks, bt_ai["mean"].values, color=C_GREEN, lw=0.4, alpha=0.25, zorder=2)
ax_top.plot(ticks, mean_bf.values, color=C_BLUE,  lw=1.8, zorder=3, label="BestFit")
ax_top.plot(ticks, mean_ai.values, color=C_GREEN, lw=1.8, zorder=3, label="AIF")
...
# Middle: raw unreachability — no smoothing, spikes stay honest
ax_mid.fill_between(ticks, if_bf.values * 100, color=C_BLUE,  alpha=0.25, zorder=2)
ax_mid.fill_between(ticks, if_ai.values * 100, color=C_GREEN, alpha=0.25, zorder=2)
ax_mid.plot(ticks, if_bf.values * 100, color=C_BLUE,  lw=0.9, zorder=3,
            label="BestFit unreachable")
ax_mid.plot(ticks, if_ai.values * 100, color=C_GREEN, lw=0.9, zorder=3,
            label="AIF unreachable")

peak = max(if_bf.max(), if_ai.max()) * 100
ax_mid.set_ylabel("Nodes with\napp unreachable (%)")
ax_mid.set_ylim(0, min(105, peak * 1.15))
...
month_labels = [f"M{m+1}" for m in range(N_MONTHS)] + [""]
...
plt.tight_layout()
out = FIG_DIR / "fig_uc2_capable_12m_bestfit_vs_aif.png"
fig.savefig(out, dpi=150, bbox_inches="tight")
plt.close(fig)
print(f"Saved {out}")

print("\nMean user_delay (finite, all phases):")
print(f"  BestFit : {bt_bf['mean'].mean():.1f} ms")
print(f"  AIF     : {bt_ai['mean'].mean():.1f} ms")

print("\nMean unreachable fraction:")
print(f"  BestFit : {if_bf.mean()*100:.2f}%")
print(f"  AIF     : {if_ai.mean()*100:.2f}%")

print(f"\nPeak unreachable:")
print(f"  BestFit : {if_bf.max()*100:.1f}%")
print(f"  AIF     : {if_ai.max()*100:.1f}%")

spike_ticks = (if_bf == 1.0).sum()
print(f"\nBestFit ticks at 100% unreachable: {spike_ticks} / {len(ticks)}"
      f"  ({spike_ticks/len(ticks)*100:.1f}%)")
